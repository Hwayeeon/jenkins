...
export default async function ProjectsPage({ params }: Props) {
  ...
  // Get views from Redis if available, otherwise default to 0 for all projects
  ...
  if (redis) {
    ...
    // Initialize with zeros if Redis is not configured
    views = allProjects.reduce((acc, p) => {
      acc[p.slug] = 0;
      return acc;
    }, {} as Record<string, number>);
  ...
  const featured = allProjects.find((project) => project.slug === "lektor")!;
  const top2 = allProjects.find((project) => project.slug === "u-reserve")!;
  const top3 = allProjects.find((project) => project.slug === "davidsonrafael")!;
